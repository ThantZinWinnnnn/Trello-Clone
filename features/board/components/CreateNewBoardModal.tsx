...
import CreateNewBoard from "@/components/utils/CreateNewBoard";
...
const CreateNewBoardModal = ({ children }: { children: React.ReactNode }) => {
  ...
  const {mutate:createBoard,isLoading} = useMutation({mutationFn:async({inputName}:inputProps)=>{
    const response = await axios.post('/api/board',{boardName:inputName});
