...
const Members = ({ boardId }: Props) => {
  ...
  const admin = useMemo(()=>members?.find((mem)=>mem?.isAdmin === true),[members])
  return (
    <Dialog>
      ...
      <DialogContent>
        ...
        <section>
          {isLoading ? (
            ...
            <section>
            <MemberInfoBtn imgUrl={admin?.User?.image!} name={admin?.User?.name!} isAdmin={admin?.isAdmin!} userId={admin?.User?.id!} adminId={admin?.User?.id!}/>
            <Separator className="my-2"/>
            <section className="flex flex-col gap-3">
            {members?.map((usr) => (
              usr?.isAdmin === false && (
                <MemberInfoBtn key={usr?.id} imgUrl={usr?.User?.image!} name={usr?.User?.name!} isAdmin={usr?.isAdmin!} userId={usr?.User?.id!} adminId={admin?.User?.id!}/>
              )
            ))}
            </section>
            </section>
